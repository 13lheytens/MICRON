...
class NeuralNetPrescriptionHistory(nn.Module):
    ...
    def forward(self, diag_codes, proc_codes, prev_med_codes):
        ...
        # Sum the embedded medical codes that are set from the previous visit, then stack into a tensor with shape (batch_size, 128).
        embedded_prev_codes = torch.stack(
            [torch.sum(self.med_embedding(med_codes), dim=0) for med_codes in prev_med_codes])

        # Combine the embedded codes into one tensor with shape (batch_size, 128*3).
...
def original_evaluate(model, val_samples):
    ...
    for sample in val_samples:
        diag_codes, proc_codes, prev_med_codes, cur_meds = sample

        # y_hat holds the predicted medication probabilities.
        y_hat = model(torch.LongTensor(diag_codes).unsqueeze(0), torch.LongTensor(proc_codes).unsqueeze(0),
                      torch.LongTensor(prev_med_codes).unsqueeze(0)).squeeze()
        y_pred = y_hat > .5
