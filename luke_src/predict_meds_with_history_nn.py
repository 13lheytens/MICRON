...
def evaluate(model, test_samples):
    ...
    for sample_patient in test_samples:
        ...
        original_patient_metrics.append(np.array(original_metrics(y.detach().numpy(), y_pred.detach().numpy(), y_hat.detach().numpy())))
...
def run_model():
    ...
    # Train the model.
    train_model(model, batch_train_samples, test_samples, optimizer, 15, criterion)
